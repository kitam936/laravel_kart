<x-app-layout>
    <x-slot name="header">
        ...
        {{-- <div class="ml-12 md:ml-12">
            <button type="button" class="w-32 h-8 bg-indigo-500 text-white md:ml-3232 hover:bg-indigo-600 rounded" onclick="location.href='{{ route('chassis_index') }}'" >Chassis_List</button>
        </div> --}}
        </div>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('chassis_update',['chassis'=>$kart->kart_id]) }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="">
                    <div class="ml-2 mr-2">
                        <x-label for="maker_id" value="Maker" class="mt-1"/>
                        ...
                        <div class="ml-0 w-52">
                                ...
                                <option value="{{ $kart->maker_id }}" @if(\Request::get('maker_id') == '0') selected @endif >{{ $kart->maker_name }}</option>
                ...
                <div class="flex">
                    ...
                    <div class="ml-2">
                        ...
                        <select name="model_year" class="w-40 bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" type="text">
                            <option value="{{ $kart->model_year}}" @if(\Request::get('model_year') == '0') selected @endif >{{ $kart->model_year}}</option>
                            @for($i = 2006; $i <= 2040; $i++)
                    ...
                    <div class="ml-2">
                        ...
                        <x-input id="purchase_date" class="bg-gray-100 text-gray-800 block mt-1 w-full" id="purchase_date" type="text" name="purchase_date" value="{{ \Carbon\Carbon::parse($kart->purchase_date)->format('Y-m-d') }}" required  />
                    </div>
                    </div>
                    <div class="relative">
                        ...
                        <x-textarea id="my_kart_info" name="my_kart_info" rows="3" required class="ml-2 w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{!! nl2br(e($kart->my_kart_info)) !!}</x-textarea>
                    </div>
                ...
                <div class="px-2 md:w-2/1 mx-auto">
                    ...
                    <div class="w-80 ml-2">
                        <x-kart-thumbnail :filename="$kart->photo1" />
                    </div>
                    <div class="w-80 ml-2">
                        <x-kart-thumbnail :filename="$kart->photo2" />
                    </div>
