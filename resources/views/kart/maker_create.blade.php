<x-app-layout>
    <x-slot name="header">
        ...
        <div class="ml-12 md:ml-12">
            <button type="button" class="w-32 h-8 bg-indigo-500 text-white md:ml-3232 hover:bg-indigo-600 rounded" onclick="location.href='{{ route('maker_index') }}'" >Maker_List</button>
        </div>
        </div>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('maker_store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="">
                    <div class="ml-2 mr-2">
                        ...
                        <x-input id="maker_name" class="bg-gray-100 text-gray-800 block mt-1 w-full" id="maker_name" type="text" name="maker_name" :value="old('maker_name')" required  />
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <x-textarea id="maker_info" name="maker_info" rows="3" required class="ml-2 w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ old('maker_info')  }}</x-textarea>
                </div>
                <div class="ml-2 mr-2">
