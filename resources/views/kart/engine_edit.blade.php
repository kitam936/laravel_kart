<x-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('engine_update',['engine'=>$engine->id]) }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="">
                    ...
                    <div class="ml-2 mr-2">
                        ...
                        <x-input id="engine_maker" class="bg-gray-100 text-gray-800 block mt-1 w-full" id="engine_maker" type="text" name="engine_maker" value="{{ $engine->engine_maker_name}}" required  />
                    </div>
                    <div class="ml-2 mr-2">
                        <x-label for="engine_name" value="エンジン名" class="mt-1"/>
                        <x-input id="engine_name" class="bg-gray-100 text-gray-800 block mt-1 w-full" id="engine_name" type="text" name="engine_name" value="{{ $engine->engine_name}}" required  />
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <x-textarea id="engine_info" name="engine_info" rows="3" required class="ml-2 w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $engine->engine_info  }}</x-textarea>
                </div>
                <div class="ml-2 mr-2">
                    <x-label for="sort_order" value="並び順" class="mt-1"/>
                    <x-input id="sort_order" class="bg-gray-100 text-gray-800 block mt-1 w-full" id="sort_order" type="text" name="sort_order" value="{{ $engine->sort_order}}" required  />
                </div>
