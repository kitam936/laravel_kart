<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex">
            ...
            <div class="ml-2 ">
                <button type="button" class="w-32 h-8 bg-green-500 text-sm text-white ml-0 hover:bg-green-600 rounded" onclick="location.href='{{ route('tire_create') }}'" >新規登録</button>
            </div>
        </div>
        <br>
            ...
            <table class="md:w-full bg-white table-auto w-full text-center whitespace-no-wrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="w-1/13 md:1/13 text-sm md:px-4 py-1 text-center"> {{ $tire->tire_maker_name }}</td>
                        <td class="w-3/13 md:3/13 text-sm md:px-4 py-1 text-indigo-500 text-center"> <a href="{{ route('tire_show',['tire'=>$tire->id]) }}" >{{ $tire->tire_name }} </a></td>
                        <td class="w-2/13 md:2/13 text-sm md:px-4 py-1 text-center"> {{$tire->tire_info}}</a> </td>
