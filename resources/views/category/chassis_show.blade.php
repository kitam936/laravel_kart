        ...
        <div class="flex ">
            <div class=" w-1/2 mt-0 flex md:ml-60">
                <div class="md:ml-2 md:ml-4">
                    <button type="button" class="w-32 h-8 text-sm bg-green-500 text-white ml-2 hover:bg-green-600 rounded" onclick="location.href='{{ route('ch_category_edit',['category'=>$category->id])}}'" >編集</button>
                </div>
            <form id="delete_{{$category->id}}" method="POST" action="{{ route('ch_category_destroy',['category'=>$category->id]) }}">
            ...
            <div class="">
                <div>
                    ...
                    <div class="relative w-full md:w-2/3 ml-2 ">
                        ...
                        <div class="pl-2 w-full h-6 text-sm items-center bg-gray-100 border rounded" id="maint_name" name="maint_name"  value="{{ $category->ch_maint_name }}">{{ $category->ch_maint_name  }}
                        </div>
                    </div>
                    <div class="relative w-full md:w-2/3 ml-2 ">
                        <x-label for="category_info" value="カテゴリー情報" class="mt-1"/>
                        <div class="pl-2 w-full h-6 text-sm items-center bg-gray-100 border rounded" id="category_info" name="category_info"  value="{{ $category->ch_maint_category_info }}">{{ $category->ch_maint_category_info  }}
                        </div>
                    </div>
                    <div class="relative w-24 ml-2 ">
