<x-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('ch_category_store') }}" >
                @csrf

                <div class="ml-2 mr-2">
                    <x-label for="ch_maint_name" value="カテゴリー名" class="mt-1"/>
                    <x-input id="ch_maint_name" class="bg-gray-100 text-gray-800 block mt-1 w-full" id="ch_maint_name" type="text" name="ch_maint_name" :value="old('ch_maint_name')" required  />
                </div>
                ...
                <div class="relative">
                    ...
                    <x-textarea id="ch_maint_info" name="ch_maint_info" rows="3" required class="ml-2 w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ old('ch_maint_info')  }}</x-textarea>
                </div>
