<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex">
            ...
            <div class="ml-2 ">
                <button type="button" class="w-32 h-8 text-sm bg-green-500 text-white ml-0 hover:bg-green-600 rounded" onclick="location.href='{{ route('eg_category_create') }}'" >新規登録</button>
            </div>
        </div>
        <br>
        ...
        <div class=" mx-auto sm:px-4 lg:px-4 border ">
            ...
            <table class="md:w-full bg-white table-auto w-full text-center whitespace-no-wrap">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="w-1/13 md:1/13 text-sm md:px-4 py-1 text-indigo-500 text-center"> <a href="{{ route('eg_category_show',['category'=>$category->id]) }}" >{{ $category->id }} </a></td>
                        <td class="w-2/13 md:2/13 text-sm md:px-4 py-1 text-center">{{$category->eg_maint_name}}</td>
                        <td class="w-2/13 md:2/13 text-sm md:px-4 py-1 text-center"> {{ $category->eg_maint_info }} </td>
                    </tr>
                    @endforeach
